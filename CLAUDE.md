...
| D13 | **Route viewer 2.0** (user, 2026-08-07). North arrow + scale bar; direction line = computed bearing + the representative trip's headsign (bearing suppressed under the 30%-displacement loop guard); **context layer** — other routes whose bbox overlaps, drawn faint behind, plus a 1-mile graticule (feed geometry only; a self-drawn OSM street layer is deferred to its own decision with measured costs); transit centers as filled **squares** with a legend, distinct from the filled-circle vehicle dot; **pinch/drag zoom 1–8× with double-tap reset** — this reopens D12's fit-only line deliberately; every glyph tappable; TripDetail and AlertDetail link into the viewer; and `scheduled` refines to **`not started yet`** vs **`no live data`** whenever a live snapshot exists (rail stays plain — D2). **Zero index-format changes.** Spec: `docs/superpowers/specs/2026-08-07-route-viewer-2.md` | the user asked for orientation, interaction and linking, and pushed back on shipping a map: a self-drawn one inside the allow-list is acceptable, so the context layer is phase 1 of that |
